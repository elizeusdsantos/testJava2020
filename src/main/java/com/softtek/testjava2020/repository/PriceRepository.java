...
public interface PriceRepository extends JpaRepository<Price, Long> {
    @Query(value = "SELECT * FROM Prices p WHERE :date BETWEEN p.start_date AND p.end_date AND p.product_id = :productId AND p.brand_id = :brandId ORDER BY price_list DESC LIMIT 1", nativeQuery = true)
